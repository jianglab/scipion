...
class BoxWizardView(tk.Frame):
    ...
    def _fillContent(self, frame):
        ...
        defaultProjectName = (datetime.datetime.now().strftime("%y%m%d") +"_"+
...
def getTemplate():
    ...
    with open(JSON_FN, 'r') as jsonFile:
        template = ''.join([line for line in jsonFile.readlines()
                            if not line.startswith('//')])  # To allow comments
    # Replace environment variables
    template = template % os.environ

    return template

def usage(errorStr, doExit):
    print('\n'+errorStr+'\n')
    print("Usage:")
    print(" -> ./scipion python scipionbox_preprocess_basic.py path/to/workflow.json")
    print("\n         option: --sym path/to/raw/data/dir")
    if doExit:
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if n > 2:
        ...
        JSON_FN = pwutils.expanduser(sys.argv[1])
        if not os.path.isfile(JSON_FN):
            usage("The input must be a JSON file.", True)
